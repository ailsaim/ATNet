...
def perceptual_loss(y_true, y_pred):
    ...
    loss_block1 = Model(inputs=vgg.input, outputs=vgg.get_layer('block5_conv3').output)
    ...
    bg = y_true[:, :, :, 8:11]
    trimap = y_true[:, :, :, 12]
    trimap = K.reshape(trimap, (-1, img_rows, img_cols, 1))
    alpha_t = y_true[:, :, :, 0]
    alpha_t = K.reshape(alpha_t, (-1, img_rows, img_cols, 1))
    y_pred_final =(1 - mask) * trimap + mask * y_pred
    c_g = alpha_t * fg + (1.0 - alpha_t) * bg
    c_p = y_pred_final * fg + (1.0 - y_pred_final) * bg
    return K.mean(K.square(loss_block1(c_g) - loss_block1(c_p)))

def over_all_add_per_loss(y_true, y_pred):
    return 0.4 * alpha_prediction_loss(y_true, y_pred) + 0.4 * compositional_loss(y_true, y_pred)+0.2*perceptual_loss(y_true, y_pred)
...
class ContentVGGRegularizer(Regularizer):
    ...
    def __call__(self, x):
        ...
        loss = self.weight * K.mean(K.square(content - generated))
        return loss
